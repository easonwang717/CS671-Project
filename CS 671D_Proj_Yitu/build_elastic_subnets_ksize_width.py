...
def main(args):
    ...
    proto_dir = os.path.join(cwd, "proto_dir_elastic_subnets_ksize_width")
    search_space_list = proxyless_search_space()
    if not os.path.exists(proto_dir):
        os.makedirs(proto_dir)

    for id in range(args.budget):
        proto = ProtoWriter("elastic-subnet-ksize-width-%d" % id)
        ...
        ops_def = change_ksize_width(search_space_list)
        ...
        file = os.path.join(proto_dir, "elastic-subnet-ksize-width-%d.prototxt" % id)
